...
// 210901 Parameter "radam" shold be independent of parameter "gamm" (A. Haga)
// 210901 Sampling momentum of the particle does not consider its binding energy (A. Haga)
// 210901 Binding energy evaluated by "GetTotalEnergy" calculated in Mean Field (A. Haga)
...
LocalQMDGroundStateNucleus::LocalQMDGroundStateNucleus( G4int z , G4int a )
: maxTrial ( 1000 )
...
, meanfield ( NULL ) 
{
   ...
   //std::cout << " LocalQMDGroundStateNucleus( G4int z , G4int a ) Begin " << z << " " << a << std::endl;
   ...
   clp = parameters->Get_clp();
    ...
    if ( z == 1 && a == 1 ) {  // Hydrogen  Case or proton primary
        SetParticipant( new LocalQMDParticipant( G4Proton::Proton() , G4ThreeVector( 0.0 ) , G4ThreeVector( 0.0 ) ) );
        ebini = 0.0;
        return;
    }
    else if ( z == 0 && a == 1 ) { // Neutron primary
        SetParticipant( new LocalQMDParticipant( G4Neutron::Neutron() , G4ThreeVector( 0.0 ) , G4ThreeVector( 0.0 ) ) );
        ebini = 0.0;
        return;
    }
   ...
   for ( int i = 0 ; i < a ; i++ )
   {
      ...
      G4ThreeVector p( 0.0 );
      G4ThreeVector r( 0.0 );
      LocalQMDParticipant* aParticipant = new LocalQMDParticipant( pd , p , r );
      SetParticipant( aParticipant );
       //G4cout << "GNucl " << i << G4endl;
   }
   ...
   //radm = radious - rada * ( gamm - 1.0 ) + radb;
    radm = radious; // JQMD, Skyrme, RMF
   rmax = 1.0 / ( 1.0 + G4Exp ( -rt00/saa ) );
    //G4cout << "GNucl " << radm << " rmax " << rmax << " radious " << radious << G4endl;
    //exit(0);
   //maxTrial = 1000;
   ...
   //std::cout << "LocalQMDGroundStateNucleus( G4int z , G4int a ) packNucleons Begin ( z , a ) ( " << z << ", " << a << " )" << std::endl;
   packNucleons();
   //std::cout << "LocalQMDGroundStateNucleus( G4int z , G4int a ) packNucleons End" << std::endl;
...
void LocalQMDGroundStateNucleus::packNucleons()
{

   //std::cout << "LocalQMDGroundStateNucleus::packNucleons" << std::endl;
   ...
   G4bool isThisOK = false;
    G4double energy_QMD = 0.0; // A.H.
    G4int Nucle_Z = GetAtomicNumber(); // A.H.
    G4int Nucle_A = GetMassNumber(); // A.H.
    G4int ccount = 1; // A.H.
...
//    Sampling Position
      ...
      if ( areThesePsOK == false ) continue;
       ...
       for (G4int ji = 0 ; ji < GetMassNumber() ; ji++ )
       {
           G4ThreeVector p0 = GetParticipant( ji )->GetMomentum();
           G4ThreeVector r0 = GetParticipant( ji )->GetPosition();
           G4String particlename = GetParticipant( ji )->GetDefinition()->GetParticleName();
           //ofs << ccount << "," << ji << "," << particlename << "," << r0.x() << "," << r0.y() << "," << r0.z() << "," << p0.x() << "," << p0.y() << "," << p0.z() << endl;
       }
       ccount += 1;
...
//    Calculate Two-body quantities
      ...
      for ( G4int i = 0 ; i < GetMassNumber() ; i++ )
      {
         ...
         //std::cout << "d_pot[i] " << i << " " << d_pot[i] << " " << participants[i]->Get4Momentum()<< std::endl;
      }
...
// Check Binding Energy
      ...
      //std::cout << "packNucleons ebinal " << ebinal*1000 << " ebini = "<< ebini << std::endl;
...
// Energy Adujstment
      ...
      while ( neaTry < maxTrial )  // Loop checking, 11.03.2015, T. Koi
      {
         ...
         //ebinal = ( totalPotentialE + ekinal ) / double ( GetMassNumber() );
          ...
          ebinal = energy_QMD;
          //energy_QMD = ebinal;
          //std::cout << "dtc: " << dtc << " ebini = "<< ebini << " energy_QMD = "<< energy_QMD*1000 << std::endl;
      }
      //std::cout << "isThisEAOK " << isThisEAOK << std::endl; 
      if ( isThisEAOK == false ) continue;
   
      isThisOK = true;
       //std::cout << "try: " << n0Try << " ebini = "<< ebini << " energy_QMD = "<< energy_QMD*1000 << std::endl;
...
G4bool LocalQMDGroundStateNucleus::samplingPosition( G4int i )
{
...
G4bool LocalQMDGroundStateNucleus::samplingMomentum( G4int i )
{
...
// 700
      ...
      //while ( ke  > edepth ) // Loop checking, 11.03.2015, T. Koi
      //{
      //   icounter++;
      //   if ( icounter > icounter_max ) {
	  //  G4cout << "Loop-counter exceeded the threshold value at " << __LINE__ << "th line of " << __FILE__ << "." << G4endl;
      ...
      //   }
         ...
         //ke = participants[i]->GetKineticEnergy();
          //ke = participants[i]->GetKineticEnergy()+participants[i]->GetMass();
          //ke = ke*ke + 2.0*participants[i]->GetMass()*d_pot[i];
          //ke = std::sqrt( ke ) - participants[i]->GetMass();
          //std::cout << "TKDB ke " << ke << " " << meanfield->GetSingleEnergy(i) - participants[i]->GetMass() << std::endl;
          //ke = meanfield->GetSingleEnergy(i);
         tkdb_i++;  
...
void LocalQMDGroundStateNucleus::killCMMotionAndAngularM()
{
...
G4bool LocalQMDGroundStateNucleus::samplingPosition_cluster( G4int z,  G4int a)
{
