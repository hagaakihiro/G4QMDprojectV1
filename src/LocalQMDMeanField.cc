...
G4double LocalQMDMeanField::GetSingleEnergy( G4int j )
{
    G4LorentzVector p4j = system->GetParticipant( j )->Get4Momentum();
...
double LocalQMDMeanField::CalDensityProfile()
{
    ...
    /*
     G4double radius0 = 0.82 * G4Pow::GetInstance()->A13( double ( A ) ) + 0.58;
     G4double dd = 0.1;
     G4int ni = int(radius0/dd*6.0);
     
     std::vector < std::vector < std::vector < G4double > > > dens;
     dens.resize( ni );
     for ( int i = 0 ; i < ni ; i++ )
     {
     dens[i].resize( ni );
     ...
     G4double rsquare = 0.0;
     
     G4double L2 = 1.0/2.0/wl;
     G4double L2fac = 1.0 / G4Pow::GetInstance()->powA ( ( 2.0 * pi * wl ) , 1.5 );
     G4double dx = 2.0;
     G4double dy = 2.0;
     G4double dz = 2.0;
     
     for ( G4int zi = 0; zi < ni ; zi++)
     {
     G4double z = -ni*dd*0.5 + dd*zi;
     dz = 6.0-dz;
     dy = 2.0;
     for ( G4int yi = 0; yi < ni ; yi++)
     {
     G4double y = -ni*dd*0.5 + dd*yi;
     dy = 6.0-dy;
     dx = 2.0;
     for ( G4int xi = 0; xi < ni ; xi++)
     {
     G4double x = -ni*dd*0.5 + dd*xi;
     dx = 6.0-dx;
     for ( G4int j = 0 ; j < A ; j++ )
     {
     G4ThreeVector rj = system->GetParticipant( j )->GetPosition();
     G4ThreeVector ri = G4ThreeVector(x,y,z);
     //G4cout << "j " << j <<" xyz " << rj << "," << ri << G4endl;
     ...
     dens[zi][yi][xi] += jbry*rh1*L2fac; //RMF-QMD
     //G4cout << "j " << j <<" xyz " << rj << "," << ri << G4endl;
     } //j
     rsquare += dy*dz*dx*(x*x+y*y+z*z)*dens[zi][yi][xi];
     //rsquare += dens[zi][yi][xi];
     } //x
     } //y
     } //z
     */
