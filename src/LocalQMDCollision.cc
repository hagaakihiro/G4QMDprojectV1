...
//
// 211028 Tentative modified in a short-lived particle production by A. Haga
// 210901 Energy difference evaluated by "GetTotalEnergy" calculated in Mean Field (A. Haga)
//
...
LocalQMDCollision::LocalQMDCollision()
: fdeltar ( 4.0 )
...
/*
LocalQMDCollision::LocalQMDCollision( const LocalQMDCollision& obj )
: fdeltar ( obj.fdeltar )
...
, fepse ( obj.fepse )
{
   ...
   if ( obj.theSystem != NULL ) {
      theSystem = new LocalQMDSystem;
      *theSystem = *obj.theSystem;
   } else {
      theSystem = NULL;
   }
   if ( obj.theMeanField != NULL ) {
      theMeanField = new LocalQMDMeanField;
...
LocalQMDCollision & LocalQMDCollision::operator= ( const LocalQMDCollision& obj)
{
   fdeltar = obj.fdeltar;
   fbcmax0 = obj.fbcmax1;
   fepse = obj.fepse;
...
void LocalQMDCollision::CalKinematicsOfBinaryCollisions( G4double dt )
{
   G4double deltaT = dt; 
...
//071101
   for ( G4int i = 0 ; i < n ; i++ )
   {
    ...
    //std::cout << theSystem->GetParticipant( i )->GetDefinition()->IsShortLived() << std::endl;
      if ( theSystem->GetParticipant( i )->GetDefinition()->IsShortLived() )
      {
          //std::cout << i << " ShortLived " << theSystem->GetParticipant( i )->GetDefinition()->GetParticleName() << " " << theSystem->GetParticipant( i )->GetPosition() << std::endl;
        //std::cout << theSystem->GetParticipant( i )->GetDefinition()->IsShortLived() << std::endl;
         ...
         // G4double eini0 = theMeanField->GetTotalPotential() + p40.e(); // before v10.07
...
*/
                   ...
                   {
                      // Append end;
                      theSystem->SetParticipant ( new LocalQMDParticipant ( (*it)->GetDefinition() , (*it)->Get4Momentum().v()/GeV , (*it)->GetPosition()/fermi ) );
                      et += (*it)->Get4Momentum().e()/GeV;
                      if ( id > 1 )
                      {
                         //081118
                         //G4cout << "LocalQMDCollision id >2; id= " << id  << G4endl;
                      }
                   }
...
//          EnergyCheck  
            ...
            //G4double efin0 = theMeanField->GetTotalPotential() + et; // before v10.07
...
//                                           *10 TK  
            ...
            else
            {
               ...
               theSystem->GetParticipant( i )->SetDefinition( pd0 ); // reset
               theSystem->GetParticipant( i )->SetPosition( r0 ); // reset
               theSystem->GetParticipant( i )->SetMomentum( p0 ); // reset
...
//081118
   ...
   for ( G4int i = 1 ; i < theSystem->GetTotalNumberOfParticipant() ; i++ )
   {
      ...
      if ( theSystem->GetParticipant( i )->IsThisHit() ) continue; 
       //std::cout << "Hits; " << theSystem->GetParticipant( i )->IsThisHit() << std::endl;
...
*/
         ...
         G4double prcm = pcm.mag();
          //std::cout << "collisions4 " << p4icm << " " << p4i << " " << p4j << std::endl;
          //exit(0);
         if ( prcm <= 0.00001 ) continue;
          //std::cout << "collisions5 " << std::sqrt(rsq) << " " << ti+tj << std::endl;
         //std::cout << "collisions5 " << std::endl;
          //std::cout << "collisions5 i " << theSystem->GetParticipant( i )->Get4Momentum() << " j " << theSystem->GetParticipant( j )->Get4Momentum() << std::endl;
...
/*
         ...
         { 
            ...
            {
               pauli_blocked = true;
               //std::cout << "LocalQMDRESULT Collsion Pauli Blocked " << std::endl;
            }
         }
         else
         {
            if ( theMeanField->IsPauliBlocked ( i ) == true || theMeanField->IsPauliBlocked ( j ) == true ) 
               pauli_blocked = false;
            //std::cout << "LocalQMDRESULT Collsion Blocked " << std::endl;
         } 
*/

/*
            G4cout << "LocalQMDRESULT Collsion initial p4 i and j "
                      << p4i << " " << p4j
                      << G4endl;
*/
//       081118
...
/*
            G4cout << "LocalQMDRESULT Collsion Really Happened between "
                      << i << " and " << j 
                      << G4endl;
            G4cout << "LocalQMDRESULT Collsion initial p4 i and j "
                      << p4i << " " << p4j
                      << G4endl;
            G4cout << "LocalQMDRESULT Collsion after p4 i and j "
                      << theSystem->GetParticipant( i )->Get4Momentum()
                      << " " 
                      << theSystem->GetParticipant( j )->Get4Momentum()
                      << G4endl;
            G4cout << "LocalQMDRESULT Collsion Diff "
                      << p4i + p4j - theSystem->GetParticipant( i )->Get4Momentum() - theSystem->GetParticipant( j )->Get4Momentum()
                      << G4endl;
            G4cout << "LocalQMDRESULT Collsion initial r i and j "
                      << ri << " " << rj
                      << G4endl;
            G4cout << "LocalQMDRESULT Collsion after r i and j "
...
*/
         ...
         } //if energetically_forbidden == false
...
G4bool LocalQMDCollision::CalFinalStateOfTheBinaryCollision( G4int i , G4int j )
{

//081103
   //G4cout << "CalFinalStateOfTheBinaryCollision " << i << " " << j << " " << theSystem->GetTotalNumberOfParticipant() << G4endl;

    G4int k = theSystem->GetTotalNumberOfParticipant(); // added by A.H.
    
   G4bool result = false;
   G4bool energyOK = false; 
   G4bool pauliOK = false; 
   G4bool abs = false;
    G4bool pion_prod = false; // added by A.H.
    G4bool pion_abs = false; // added by A.H.
   LocalQMDParticipant* absorbed = NULL;
...
//071031
   ...
   //G4double eini0 = epot + p4i.e() + p4j.e();  // before v10.07
    G4double eini = theMeanField->GetTotalEnergy(); // R-JQMD, Skyrme, RMF
...
//071031
   ...
   for ( G4int iitry = 0 ; iitry < 4 ; iitry++ )
   {
      ...
      G4LorentzVector p4jx_new;
       G4LorentzVector p4kx_new(G4ThreeVector(0,0,0) , 0 ); // added by A.H.
      G4KineticTrackVector* secs = NULL;
      secs = theScatterer->Scatter( kt1 , kt2 );

      //std::cout << "LocalQMDSCATTERER BEFORE " << kt1.GetDefinition()->GetParticleName() << " " << kt1.Get4Momentum()/GeV << " " << kt1.GetPosition()/fermi << std::endl;
      //std::cout << "LocalQMDSCATTERER BEFORE " << kt2.GetDefinition()->GetParticleName() << " " << kt2.Get4Momentum()/GeV << " " << kt2.GetPosition()/fermi << std::endl;
      ...
      if ( secs )
      {
         ...
         if (  secs->size() == 2 )
         {
                ...
                = secs->begin() ; it != secs->end() ; it++ )
            {
                  ...
                  //theSystem->GetParticipant( j )->SetMomentum( p4jx_new.v() );
                   ...
                   if((*it)->GetDefinition()->IsShortLived())
                   {
                       ...
                       for(G4KineticTrackVector::iterator jter=dec->begin(); jter != dec->end(); jter++)
                       {
                           ...
                           if(ita == 0)
                           {
                               theSystem->SetParticipant( new LocalQMDParticipant( (*jter)->GetDefinition() , (*jter)->Get4Momentum().v()/GeV , (*jter)->GetPosition()/fermi ) );
               ...
               //std::cout << "LocalQMDSCATTERER AFTER " << (*it)->GetDefinition()->GetParticleName() << " " << (*it)->Get4Momentum()/GeV << std::endl;
               iti++;
            }
         }
         else if ( secs->size() == 1 )
...
//081118
            abs = true;
            //G4cout << "LocalQMDCollision pion absrorption " << secs->front()->GetDefinition()->GetParticleName() << G4endl;
...
//081118
         if ( secs->size() > 2 ) 
         {
            ...
            G4cout << "LocalQMDCollision secs size > 2;  " << secs->size() << G4endl;

            for ( G4KineticTrackVector::iterator it 
                = secs->begin() ; it != secs->end() ; it++ )
            {
               G4cout << "LocalQMDSCATTERER AFTER " << (*it)->GetDefinition()->GetParticleName() << " " << (*it)->Get4Momentum()/GeV << G4endl;
            }
...
//071031
      ...
      { 
         ...
         //theMeanField->Cal2BodyQuantities( j );
          theMeanField->Update();
          //theMeanField->Cal2BodyQuantities( );
      } 
      else
      {
      ...
      //G4double efin0 = epot + p4ix_new.e() + p4jx_new.e() + p4kx_new.e(); // before v10.07
       G4double efin = theMeanField->GetTotalEnergy();  // R-JQMD, Skyrme, RMF
...
//071031
       G4double fepse_change = fepse; // Added by A.H.
       if(pion_prod || pion_abs) fepse_change = fepse*10; // Added by A.H.
      if ( std::abs ( eini - efin ) < fepse_change ) // Modified by A.H.
      {
         // Collison OK 
          //if(pion_prod || pion_abs) G4cout << "collisions6 " << std::abs ( eini - efin )*1000 << " orig " << std::abs ( eini0 - efin0 )*1000 << G4endl;
          //G4cout << "collisions6 " << std::abs ( eini - efin )*1000 << G4endl;
      ...
      else
      {
         //if(pion_prod || pion_abs) G4cout << "EnergyNotOK " << std::abs ( eini - efin )*1000 << " orig " << std::abs ( eini0 - efin0 )*1000 << G4endl;
          ...
          if(std::abs ( eini - efin )*1000 > 20)
          {
           ...
           << p4i + p4j - theSystem->GetParticipant( i )->Get4Momentum() - theSystem->GetParticipant( j )->Get4Momentum()
           << G4endl;
              G4cout << "LocalQMDRESULT Particle Name "
         ...
         else if ( pion_prod ) // added by A.H.
          {
              //G4cout << "TKDB reinsert j " << G4endl;
...
G4bool LocalQMDCollision::CalFinalStateOfTheBinaryCollisionJQMD( G4double sig , G4double cutoff , G4ThreeVector pcm , G4double prcm , G4double srt , G4ThreeVector beta , G4double gamma , G4int i , G4int j )
{
...
// std::cout << "Collision pcm " << i << " " << j << " " << pcm << std::endl;
   ...
   //G4double epot = theMeanField->GetTotalPotential();
   //G4double eini0 = epot + p4i.e() + p4j.e();
    G4double eini = theMeanField->GetTotalEnergy();
...
//
      ...
      //epot = theMeanField->GetTotalPotential();
      //G4double efin0 = epot + pi_new_e + pj_new_e ;
