...
// 210801 Total energy evaluated by Lorentz covariant version by A. Haga
...
void LocalQMDNucleus::CalEnergyAndAngularMomentumInCM()
{
   ...
   for ( G4int i= 0; i < n ; i++ ) 
   {
      ...
      //es[i] = std::sqrt ( G4Pow::GetInstance()->powN ( GetParticipant( i )->GetMass() , 2 ) + pcm[i]*pcm[i] );
       ...
       totalpote += nucpote;
       //G4cout << "nucl i " << i << " " << nucpote << " " << es[i] << G4endl;
      rcm[i] = ri + trans*beta;
...
// Total (not per nucleion ) Binding Energy 
   ...
   if ( excitationEnergy < 0 ) excitationEnergy = 0.0;
    //G4cout << "excitationEnergy in MeV " << excitationEnergy*1000 << " bindingEnergy in MeV " << bindingEnergy*1000 << G4endl;
