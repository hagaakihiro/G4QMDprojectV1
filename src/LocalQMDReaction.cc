...
//
// 210801 Fix for nucleon deplication (another bug fixed in v10.7)
//          system->clear() in line 735
// 210801 Allowing to simlate nucleon-nucleon, pion-nucleon scattering
//          pion is OK in the Ratherford parameter setting.
//
...
, frag ( false )
{
    G4cout << "LocalQMDReaction::LocalQMDReaction" << G4endl;
   //090331
...
G4HadFinalState* LocalQMDReaction::ApplyYourself( const G4HadProjectile & projectile , G4Nucleus & target )
{
   ...
   else
   {
    ...
    //G4cout << "projectile: " << proj_Z << ", " << proj_A << G4endl;
    //exit(0);
...
//   G4Element* G4NistManager::FindOrBuildElement( targ_Z );
   ...
   G4LorentzVector targ4p( G4ThreeVector( 0.0 ) , targ_pd->GetPDGMass()/GeV );
    //G4cout << "LocalQMDReaction::ApplyYourself 3" << G4endl;
    //G4cout << targ4p << G4endl;
   ...
   {
      icounter++;
       //if(icounter != 1) G4cout << "Reaction trial: " << icounter << G4endl;
       //G4cout << "Reaction trial: " << icounter << G4endl;
...
// impact parameter 
      ...
      G4double b = bmax * std::sqrt ( G4UniformRand() );
       ...
       bvalue = b;
       //G4double b = bmax * 5;
      //G4cout << "LocalQMDRESULT bmax = " << bmax << " fm, b = " << b  << " fm " << G4endl;
//071112
      //G4double b = bmax;
...
// Projectile
        ...
        || projectile.GetDefinition()->GetParticleName() == "neutron" )
      {
          ...
          //for ( G4int ittt = 0 ; ittt < 100 ; ittt++ )
          //{
              
         proj_Z = proj_pd->GetAtomicNumber();
         proj_A = proj_pd->GetAtomicMass();

          //G4cout << "Projectile GroundState Z A: " << proj_Z << "," << proj_A <<G4endl;
         ...
         proj->CalEnergyAndAngularMomentumInCM();
          //G4cout << "meanField->GetPotential() " << meanField->GetPotential(0) << G4endl;
          
          /*
          // radius check in ground state nucleus
          G4double rsquare = meanField->CalDensityProfile();
          G4double csquare = meanField->CalChargeDensityProfile();
          G4double radius0 = 0.82 * G4Pow::GetInstance()->A13( double ( proj_A ) ) + 0.58;
          G4cout << "R-square " << std::sqrt(rsquare) << ", ex. " <<  radius0 << G4endl;
          G4cout << "C-square " << std::sqrt(csquare) << G4endl;
          //exit(0);
           */
          // stability check in ground state nucleus
          G4int itt_max = 0;
          G4double del_itt = 1;
          G4double be_min = 10;
          G4double be_max = -10;
          G4double be_mean = 0.0;
          G4double be_ini = meanField->GetTotalEnergy()/ double ( proj_A );
          for ( G4int itt = 0 ; itt < itt_max ; itt++ )
          {
              //G4cout << " do Propagate " << i << " th time step. " << G4endl;
              meanField->DoPropagation( del_itt );

              meanField->Cal2BodyQuantities();
              //G4double totalPotentialE = meanField->GetTotalPotential();

              //G4ThreeVector ekinal(0.0,0.0,0.0);
              //G4ThreeVector p0check(0,0,0);
              G4double etotal = 0.0;
              //G4double potetotal = 0.0;
              //for ( int i = 0 ; i < proj_A ; i++ )
              //{
              //    ekinal = proj->GetParticipant( i )->GetMomentum();
              //    p0check += ekinal;
              //    G4double emass = proj->GetParticipant( i )->GetMass();
              //    //ekinal += emass;
              //    //ekinal = ekinal * ekinal;
              //    G4double nucpote = meanField->GetPotential(i);
              //    G4double e0 = std::sqrt(ekinal*ekinal+emass*emass + 2*emass*nucpote) - emass;
              //    etotal += e0;
              //    potetotal += nucpote;
              //    //G4cout << " pote " << nucpote << " momentum "<< ekinal << " p0check " << p0check << G4endl;
              //  G4cout << proj->GetParticipant( i )->GetPosition().mag() <<G4endl;
              //}
          //exit(0);
              etotal = meanField->GetTotalEnergy();
              
              //G4double energy = ( totalPotentialE + ekinal ) / double ( proj_A );
              G4double energy = ( etotal ) / double ( proj_A );
              
              if(be_min > energy) be_min = energy;
              if(be_max < energy) be_max = energy;
              be_mean += energy;
              //G4cout << " Propagated energy: " << energy*1000 << " min " << be_min << " max " << be_max << G4endl;

          }
              

          //rsquare = meanField->CalDensityProfile();
          //G4cout << "R-square " << std::sqrt(rsquare) << ", ex. " <<  radius0 << G4endl;
          if(itt_max != 0)
          {
              be_mean = be_mean/itt_max;
              G4cout << be_mean*1000 << " " << be_min*1000 << " " << be_max*1000 << " " << be_ini*1000 << G4endl;
          }
          //exit(0);
          //} //ittt
          //exit(0);
...
// Target
      ...
      //migrate to integer A and Z (GetN_asInt returns number of neutrons in the nucleus since this)
       ...
       char buffer[20];
       int n0 = sprintf(buffer, "radius_z%d_a%d.csv", iz, ia);
       int samp_num = 100;
       ofstream ofs0(buffer);
       ofs0 << "ip" << "," << "rsquare" << "," << "csquare" << "," << "empirical" << endl;
       G4cout << "Target GroundState Z A: " << iz << "," << ia <<G4endl;
       for ( G4int ip = 0 ; ip < samp_num ; ip++ )
       {
           ...
           G4double rsquare = meanField->CalDensityProfile();
           G4double csquare = meanField->CalChargeDensityProfile();
           G4double radius0 = 0.82 * G4Pow::GetInstance()->A13( double ( ia ) ) + 0.58;
           G4cout << "R-square " << std::sqrt(rsquare) << ", ex. " <<  radius0 << G4endl;
           G4cout << "C-square " << std::sqrt(csquare) << G4endl;
       
           ofs0 << ip << "," << std::sqrt(rsquare) << "," << std::sqrt(csquare) << "," << radius0 << endl;
       }
       // target radius sampling -- end
       exit(0);
       */
       LocalQMDGroundStateNucleus* targ = new LocalQMDGroundStateNucleus( iz , ia );
       //targ->ShowParticipants();
...
// Time Evolution 
   ...
   for ( G4int i = 0 ; i < maxTime ; i++ )
   {
      ...
      {
         //G4cout << i << " th time step. " << G4endl;
          //Writing the coordinate of each particle
         //system->ShowParticipants();
          G4double etotal0 = 0.0;
          G4double potetotal0 = 0.0;
          G4ThreeVector p0total(0,0,0);
          G4ThreeVector p0check(0,0,0);
          G4ThreeVector r0check(0,0,0);
          G4double totalmass = 0;
          //for ( G4int ji = 1; ji < system->GetTotalNumberOfParticipant(); ji++ )
          //{
          //    G4ThreeVector p0 = system->GetParticipant( ji )->GetMomentum();
          //    p0total += p0/(system->GetTotalNumberOfParticipant()-1);
          //}
          //G4cout << " p0total " << p0total << G4endl;
          for ( G4int ji = 1; ji < system->GetTotalNumberOfParticipant(); ji++ )
          {
              G4ThreeVector p0 = system->GetParticipant( ji )->GetMomentum() -p0total;
              p0check += p0;
              G4ThreeVector r0 = system->GetParticipant( ji )->GetPosition();
              r0check += r0;
              G4double mass0 = system->GetParticipant( ji )->GetMass();
              G4double nucpote = meanField->GetPotential( ji );
              potetotal0 += nucpote;
              G4double ener0 = std::sqrt (mass0*mass0 + p0*p0 + 2.0*mass0*nucpote) - mass0;
              etotal0 += ener0;
              G4String particlename = system->GetParticipant( ji )->GetDefinition()->GetParticleName();
              //ofs << i << "," << ji << "," << particlename << "," << r0.x() << "," << r0.y() << "," << r0.z() << "," << p0.x() << "," << p0.y() << "," << p0.z() << endl;
              //ekinal_0 += system->GetParticipant( ji )->GetKineticEnergy();
              //G4cout << " pote " << nucpote << " momentum "<< p0 << " p0check " << p0check <<G4endl;
          }
          //G4cout << " do Propagate " << i << " th time step. " << etotal0*1000 << " pote " << potetotal0 << G4endl;
          etotal0 = (etotal0)/(system->GetTotalNumberOfParticipant()-1);
          //G4cout << " do Propagate " << i << " th time step. " << etotal0*1000 << " pote " << potetotal0 << G4endl;
          //G4cout << " r0check " << r0check << " target "<< system->GetParticipant( 0 )->GetPosition() << G4endl;
          //exit(0);
      }
        */
      //system->ShowParticipants();
...
//       InelasticLike with Collision
      ...
      /*
      G4cout << "Collision NO: " << system->GetNOCollision() << G4endl;
      for ( G4int i = 0 ; i < int ( nucleuses.size() ) ; i++ )
      {
           meanField->SetNucleus( nucleuses[i] );
...
*/
      ...
      //std::cout << "LocalQMDRESULT nucleus nucleus_e " << nucleus_e << " " << (*it)->Get4Momentum() <<std::endl;
...
*/
   ...
   for (G4int i = 0; i < G4int(theParticleChange.GetNumberOfSecondaries() ); i++)
   {
     //G4cout << "Particle : " << theParticleChange.GetSecondary(i)->GetParticle()->GetParticleDefinition()->GetParticleName() << G4endl;
       G4double ei = theParticleChange.GetSecondary(i)->GetParticle()->Get4Momentum().e();
       esum += ei;
       //G4cout << "Momentum4 : " << theParticleChange.GetSecondary(i)->GetParticle()->Get4Momentum() << G4endl;
    ...
    //if(std::abs(init_elab - esum) > 100) G4cout << "TotalEnergy, " << esum << ", elab, " << init_elab << ", Diff, " << init_elab - esum << ", b-val, " << bvalue << G4endl;
    //G4cout << "TotalEnergy, " << esum << ", elab, " << init_elab << ", Diff, " << init_elab - esum << ", b-val, " << bvalue << G4endl;
